 ...
 *
 * Two parallel implementations of the same three exercises:
 ...
 * axi_fifo_mm_s, which has a different register layout — TDFR/TDFD/RLR/RDFD
 * vs simple_fifo's push/pop@+0), so test_fifo() here still goes through
 * direct MMIO via local helpers. The other two tests use the BSP drivers.
 *
 * Swap test_* → mem_test_* in main() to run the MMIO path instead.
 *
...
static int test_dma_fifo(void)
{
    ...
    /* Submit S2MM (receiver) first so the AXIS FIFO has a ready sink. */
...
int main(void)
{
    ...
    xil_printf("\r\n=========================================\r\n");
    xil_printf("kr260_hw_bm bare-metal test (Xilinx drivers)\r\n");
    xil_printf("=========================================\r\n");

    if (init_devices() != 0) {
        xil_printf("Device initialization failed — aborting\r\n");
        Xil_DCacheDisable();
        Xil_ICacheDisable();
        return 1;
    }
